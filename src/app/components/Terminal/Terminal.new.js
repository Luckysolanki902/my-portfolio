...
const Terminal = ({ initialMessage = "Hey, type help for a list of commands or type ai to start chatting with me", onMinimize }) => {
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (
        ...
        !isMinimized &&
        !event.target.closest('.floatingButton') // Don't minimize if clicking the floating button
      ) {
        setIsMinimized(true);
  ...
  const handleMinimize = (e) => {
    ...
    if (onMinimize) onMinimize(newMinimizedState);
  };
...
Mobile: +91 9027495997`;
  ...
  // Maximize terminal when clicked if minimized
  ...
  return (
    ...
    >
      <div className={styles.terminalHeader} onClick={handleHeaderClick}>
        <div className={styles.terminalControls}>
          <span className={styles.terminalControl}></span>
          <span className={styles.terminalControl}></span>
          <button 
            className={`${styles.terminalControl} ${styles.minimizeControl}`}
        ...
        <div className={styles.terminalActions}>_</div>
      </div>
      
      <div className={`${styles.terminalBody} ${isMinimized ? styles.hidden : ''}`} onClick={focusInput} ref={terminalRef}>
